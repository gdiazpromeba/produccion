MejoresFichasEnMonto = Ext.extend(Ext.Panel, {
  constructor : function(config) {
    
    var storeCompartido= new Ext.data.Store({
      proxy : new Ext.data.HttpProxy({
        url : '/produccion/svc/conector/estadisticas.php/mejoresFichasEnMonto'
      }),
      autoLoad: false,
      ...
      autoDestroy : false
		...
		MejoresFichasEnMonto.superclass.constructor.call(this, Ext.apply({
        iconCls:'chart',
        title: 'Mejores fichas en monto',
        frame:true,
        width:500,
        height:300,
        layout:'border',
            ...
            items:[
              {xtype: 'panel', itemId: 'busqueda', flex: 1, layout: 'form',
                items:[
                  ...
                  {fieldLabel: 'Fichas a mostrar',  itemId: 'cuantas',  xtype: 'spinnerfield', width: 45, value: 8, minValue: 1, maxValue: 25, allowDecimals: false, accelerate: false}
                ],
                buttons:[
           ...
           {xtype: 'columnchart', region: 'center',  itemId: 'grafico',
            ...
            tipRenderer : function(chart, record, index, series){
                if(series.yField == 'montoPedido'){
                    return Ext.util.Format.number(record.data.unidadesPedidas, '0,0') + ' pedido de la ficha ' + record.data.ficha;
                }else{
                    return Ext.util.Format.number(record.data.unidadesRemitidas, '0,0') + ' remitido de la ficha ' + record.data.ficha;
                }
            },
            chartStyle: {
                padding: 10, 
